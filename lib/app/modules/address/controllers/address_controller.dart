...
class AddressController extends GetxController {
  ...
  void updatePosition(CameraPosition position) async {
    ...
    update();
    await getAddressFromGeocodeData(
  ...
  deleletAddress(int id) {
    ...
    update();
    server.deleteRequest(endPoint: APIList.addressDelete! + id.toString()).then(
      (response) {
  ...
  Future<bool> checkBranchByLatLong({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(data);
        branchData = BranchData.fromJson(data);
        return true;
